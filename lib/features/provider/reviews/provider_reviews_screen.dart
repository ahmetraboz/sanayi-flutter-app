          ...
          if (hasReply) ...[
            ...
            GestureDetector(
              onTap: () => setState(() {
                ...
                if (_showReplyForm) _controller.text = r.providerReply ?? '';
              }),
              child: Text(
                _showReplyForm ? 'İptal' : 'Yanıtı Düzenle',
                style: const TextStyle(fontSize: 12, color: AppColors.blue600, fontWeight: FontWeight.w600),
              ),
            ),
          ] else if (!_showReplyForm) ...[
            const SizedBox(height: 10),
            GestureDetector(
              onTap: () => setState(() => _showReplyForm = true),
              child: const Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(Icons.reply_outlined, size: 14, color: AppColors.blue600),
